...
@if($variant === 'link')
    <a href="{{ $href }}" class="btn btn-ghost relative " . {{ $style }}>
        ...
        @if ($label) <span @class(['my-0', $size, ])> {{ $label }} </span> @endif
...
@else 
    <button @class([ 'btn', 'btn-' . $variant, $style, 'btn-disabled' => $disabled, ])>
        ...
        @if ($label) <span @class([ $size, ])> {{ $label }} </span> @endif

        @if ($trailing) <span class="material-symbols-outlined !text-[20px]">{{ $trailing }}</span> @endif
    </button>
@endif